...
<body>
	...
	<div class="warn__warning clearfix">
		<div class="warning__wrap">
			...
			<ul>
				<li data-con="data/warning.html" class="con__header--active">围栏通知</li>
			</ul>
			</a> </header>
		...
		<div class="warnDetails__wrap">
			...
			<ul>
				<li data-con="${basePath }/data/warnDetails.jsp"
					class="con__header--active">围栏通知</li>
				...
				<div class="con__main__table">
					<table id = "alert_table">
						...
						<tbody class="alertBind" id = "showSelectAlerts">
							<c:choose>
								<c:when test="${empty geomessages}">
									<tr>
										<td style="background-color:transparent; color:inherit;">无数据！</td>
									</tr>
								</c:when>
								<c:otherwise>
									<c:forEach items="${geomessages}" var="alert" varStatus="status">
										<tr>
	...
	function searchAlert(){
		...
		var endTime = $.trim($("#endTime").val());
		$.ajax({
					url : "${pageContext.request.contextPath}/pc/alert/showGeomessagetsInTwoTimeAndContainerId.do",
					data : {
						"containerId" : containerId,
						"startTime"  : startTime,
						"endTime"    : endTime
	...
	<script type="text/javascript">
		...
		$(".alertBind").on("click","tr",function(){
			...
			$.ajax({
				url:"${pageContext.request.contextPath}/pc/alert/modifyAlertReadStateByGeomessagetId.do",
